...
# Views (i.e. MVC controllers) used to handle HTTP requests and responses for bookmarks

from flask import render_template, flash, redirect, url_for, request, abort
from flask_login import login_required, current_user

from . import bookmarks  # Bookmark blueprint
from .forms import BookmarkForm  # Own forms
from .. import db  # Parent db
from ..models import User, Bookmark, Tag  # Parent model
...
def edit(bookmark_id):
    ...
    if current_user != bookmark.user:
        abort(403)
    form = BookmarkForm(obj=bookmark)  # Pre fill from db if no data
